...
namespace SDKSample.HttpClientSample
{
    ...
    public sealed partial class Scenario14 : Page, IDisposable
    {
        ...
        private void InitializeHttpClient()
        {
            ...
            httpClient = new HttpClient(filter); // We need a new instance every time "start" is clicked, moving this.
        ...
        {
            if (hasRunBefore) InitializeHttpClient();
            ...
            // we need a username and password for HTTP Digest
            if (UserField.Text.Length == 0 || PasswordField.Text.Length == 0)
            {
                rootPage.NotifyUser("A username and password must be specified.", NotifyType.ErrorMessage);
                return;
            }
